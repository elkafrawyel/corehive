...
class ProductDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProductDetailsController>(
      init: ProductDetailsController(product: product, productRepo: Get.find()),
      builder: (controller) => Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              icon: Icon(
                ProductDetailsController.to.product.isWishlisted
                    ? Icons.favorite
                    : Icons.favorite_border,
                color: ProductDetailsController.to.product.isWishlisted
                    ? Colors.red
          ...
          child: Column(
            ...
            children: [
              ...
              Stack(
                ...
                children: [
                  ImageSliderSection(product: product, heroTag: heroTag),
                  ImageIndicatorsSection(product: product),
                ],
              ),
              Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    PriceSection(product: product, heroTagTitle: heroTagTitle),
                    ...
                    AppButton(
                      text: 'Add to Cart',
                      onPressed: controller.addToCartWithQuantity,
                    ),
                    const SizedBox(height: 24),
                    DescriptionSection(product: product),
                    const SizedBox(height: 16),
                    BuyerSection(product: product),
                    const SizedBox(height: 24),
                    PropertiesSection(product: product),
                    const SizedBox(height: 24),
